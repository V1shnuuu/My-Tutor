...
export async function* chat(token: string, message: string, history: { role: string; content: string }[], prevLang: Lang | null, signal?: AbortSignal): AsyncGenerator<ChatEvent> {
  const r = await check(
    await fetch(`${API}/chat`, { method: "POST", headers: headers(token), body: JSON.stringify({ message, history, prev_lang: prevLang }), signal }),
  );
